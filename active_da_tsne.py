...
class active_da_digits():
    ...
    def train_source(self):
        ...
        for epoch in range(self.source_training_epoch):
            ...
            if best_acc < acc_t:
                best_acc = acc_t
                torch.save(self.fea, './saved_model/source_only'+self.source_name+self.target_name+'fea.pt')
                torch.save(self.clf, './saved_model/source_only'+self.source_name+self.target_name+'clf.pt')
    ...
    def WL_active_use_weighted_loss(self, uncertainty_method):
        ...
        for epoch in range(self.acitve_epoch):
            ...
            if best_acc < acc_t:
                best_acc = acc_t
                torch.save(self.fea, './saved_model/'+self.source_name+self.target_name+'fea.pt')
                torch.save(self.clf, './saved_model/'+self.source_name+self.target_name+'clf.pt')
                print('model saved succsessfully')
